...
class HomeworkViewModel {
    ...
    struct Input {
        var searchBarEvent: ControlEvent<Void>
        var searchText: ControlProperty<String>
        var selectedCellEvent: ControlEvent<Person>
    }
    
    struct Output {
        var personList:BehaviorSubject<[Person]>
        var selectedPersonList:BehaviorSubject<[String]>
    }
    ...
    func transform(input: Input) -> Output {
        ...
        let itemList = BehaviorSubject<[Person]>(value: sampleUsers)
        let selectedPersonList = BehaviorSubject<[String]>(value: [])
        
        input.searchBarEvent
            .withLatestFrom(input.searchText)
            .bind(with: self) { owner, text in
                let newPerson = Person(name: text, email: "steven.brown@example.com", profileImage: "https://randomuser.me/api/portraits/thumb/women/25.jpg")
                var newList = try! itemList.value()
                newList.insert(newPerson, at: 0)
                
                itemList.onNext(newList)
            }
            .disposed(by: disposeBag)
        
        input.selectedCellEvent
            .bind(with: self) { owner, person in
                print("사라아암", person)
                var newItems = try! selectedPersonList.value()
                newItems.insert(person.name, at: 0)
                selectedPersonList.onNext(newItems)
            }
            .disposed(by: disposeBag)
        
        
        
        return Output(personList: itemList, selectedPersonList: selectedPersonList)
    }
}
